...
def gtmf_to_markdown(gtmf: GTMF | dict[str, Any]) -> str:
    ...
    past_history = (
        history.get("Past_Medical_History", "not provided") if history else "not provided"
    )
            ...
            if isinstance(tx, dict):
                ...
                if meds:
                    ...
                    for med in meds:
                        if isinstance(med, dict):
                            lines.append(
                                f"- {med.get('name', '')} ({med.get('dosage', 'dose not specified')})"
                            )
                        else:
                            lines.append(f"- {med}")
                lines.append("")
        ...
        if current_section == "Patient Information":
            ...
            if not line_stripped.startswith("-"):
                ...
                if current_complaint == "not provided":
                    current_complaint = ""
                gtmf["Additional_Context"]["Chief_Complaint"] = (
                    current_complaint + " " + line_stripped
                ).strip()
            ...
            if not line_stripped.startswith("-"):
                current_history = gtmf["Context_Fields"]["Medical_History"].get(
                    "Past_Medical_History", ""
                )
                if current_history == "not provided":
                    current_history = ""
                gtmf["Context_Fields"]["Medical_History"]["Past_Medical_History"] = (
                    current_history + " " + line_stripped
                ).strip()

        elif current_section == "Allergies":
            if not line_stripped.startswith("-"):
                allergies_str = line_stripped.strip()
                gtmf["Context_Fields"]["Allergies"] = [
                    a.strip() for a in allergies_str.split(",") if a.strip()
                ]
            ...
            if current_subsection and current_subsection != "Structured Clinical Data":
                ...
                if "**Details**:" in line_stripped:
                    ...
                    current_treatment["medications"].append(
                        {
                            "name": med_name,
                            "dosage": "not provided",
...
def _parse_diagnosis(line: str) -> dict[str, str]:
    ...
    diagnosis = {"primary": primary, "notes": "not provided"}
...
def _parse_icd_entry(line: str) -> dict[str, Any]:
    ...
    if match:
        entry = {"icd9_code": match.group(1).strip(), "description": match.group(3).strip()}
        if match.group(2):
...
def _parse_prescription(line: str) -> dict[str, Any]:
    ...
    if parts:
        return {"drug": parts[0], "dose_val_rx": parts[1] if len(parts) > 1 else ""}
    return None
